...
function providerError(message: string, details: Readonly<Record<string, unknown>> = {}): AgnesError {
...
export class OpenMeteoWeatherAdapter implements WeatherPort {
  ...
  async getCurrent(query: WeatherQuery): Promise<WeatherSnapshot> {
    ...
    if (Number.isNaN(observedAt.getTime())) {
      throw providerError('Malformed Open-Meteo field: current.time', { field: 'current.time' });
    }
    ...
    const rainProbability = requiredNumber(rainProbabilities[index], 'hourly.precipitation_probability');
